...
const mdPath = path.join("C:", "Users", "Uday", "Downloads", "JS_DOM_MCQ_Chapters_21_to_30.md");
...
if (!last || last.chapter !== 20) {
  console.error(`Expected mcqData.json to end at chapter 20, last chapter is ${last?.chapter}`);
  process.exit(1);
}

const merged = existing.concat(out);
fs.writeFileSync(mcqPath, JSON.stringify(merged, null, 2) + "\n", "utf8");
console.log(`Appended ${out.length} questions (ch.21–30). Total MCQs: ${merged.length}`);
